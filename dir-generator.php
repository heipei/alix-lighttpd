...
<html lang="en">
	<head>
		<meta charset="utf-8">
		<title>Motherload - Index of /'.$vpath.'</title>
		<link rel="stylesheet" type="text/css" href="/.icons/dir-listing.css" /> 
	</head>
	<body>
';
...
if ($display_header)
{
	...
	if (is_file($path.'/HEADER.txt'))
	{
		readfile($path.'/HEADER.txt');
	}
}
...
if($handle = @opendir($path)) {
	while(($item = readdir($handle)) !== false) {
		...
		elseif(is_file($path.'/'.$item)) {
			...
			if ($hide_header)
			{
				if ($item === 'HEADER' || $item === 'HEADER.txt')
				{
					continue;
				}
			}
			if ($hide_readme)
			{
				if ($item === 'README' || $item === 'README.txt')
				{
					continue;
				}
			}
			...
			$filelist[] = array(
				'name'=> $item,
				'size'=> `/usr/bin/stat -c%s $path/$item`,
				'modtime'=> filemtime($path.'/'.$item),
...
print "	<div class='foot'>". $_ENV['SERVER_SOFTWARE'] . " on Motherload</div>
	</body>
	</html>";
